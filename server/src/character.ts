...
export const character: Character = {
  ...
  plugins: [
    // removed,
    ...(process.env.ANTHROPIC_API_KEY?.trim() ? [// removed] : []),
    ...(hasOpenAICompatibleKey ? [// removed] : []),
    ...(process.env.TELEGRAM_BOT_TOKEN?.trim() ? [// removed] : []),
    ...(!process.env.IGNORE_BOOTSTRAP ? [// removed] : []),
  ],
  settings: {
    secrets: {},
  },
